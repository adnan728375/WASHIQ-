...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
    return async function (event, message) {
        ...
        // =====================
        // ✅ Mentions + Reply Patch (Fixed by WASHIK)
        // - Allows: message.reply({ body, mentions, attachment, ... })
        // - Creates: event._mentionIDs for commands (from mentions / reply / @name fallback)
        // =====================

        // 1) Upgrade message.reply to support object payload (mentions/attachment/etc.)
        if (message && typeof message.reply === "function" && !message.__washikReplyPatched) {
            const oldReply = message.reply.bind(message);

            message.reply = function (payload, callback) {
                try {
                    // old style: message.reply("text")
                    if (typeof payload === "string") {
                        return oldReply(payload, callback);
                    }

                    // new style: message.reply({ body, mentions, attachment, ... })
                    if (payload && typeof payload === "object") {
                        const msg = payload;

                        // Ensure msg.body is a string
                        if (typeof msg.body !== "string") msg.body = String(msg.body || "");

                        // Use api.sendMessage directly so mentions works reliably
                    ...
                    return oldReply(String(payload), callback);
                } catch (e) {
                    return oldReply("❌ Reply error.", callback);
                }
            };

            message.__washikReplyPatched = true;
        }

        // 2) Resolve mention targets into event._mentionIDs
        async function __resolveMentionsWashik() {
            const bodyText = typeof event.body === "string" ? event.body : "";

            // If Facebook already provides mentions
            const mentionsObj = event.mentions || {};
            const mentionIDs = Object.keys(mentionsObj);
            if (mentionIDs.length) {
                event._mentionIDs = mentionIDs;
                return;
            }

            // If user replied to someone -> treat as mention target
            if (event.messageReply?.senderID) {
                event.mentions = { [event.messageReply.senderID]: "" };
                event._mentionIDs = [event.messageReply.senderID];
                return;
            }

            // Fallback: try match "@name" inside group only
            if (!event.isGroup || !bodyText.includes("@")) return;

            const tagMatch = bodyText.match(/@([^\s\n]+)/);
            if (!tagMatch) return;
            ...
            try {
                ...
                let foundID = Object.keys(nicknames).find(id =>
                    String(nicknames[id] || "").toLowerCase().includes(tagName)
                );
        ...
        // Call resolver early so commands can use event._mentionIDs
        await __resolveMentionsWashik();

        // 3) Helper: build mentions array for api.sendMessage
        function buildMentions(bodyText, id, tagText) {
            const fromIndex = String(bodyText).indexOf(tagText);
            if (!id || fromIndex === -1) return [];
            return [{ id, tag: tagText, fromIndex }];
        }
